...
`define SDFFILE "./build/huffman_syn.sdf"  // Modify your sdf file name
  ...
  if (reset) begin
    ...
    if(CNT_valid == 1'b1) begin
      if (CNT_G == CNT_EXP) begin     // flag1 1 means PASS, 0 means ERROR
        ...
        $display("CNT:%d,%d,%d,%d,%d,%d , GOLDEN:%d,%d,%d,%d,%d,%d\n",CNT[1],CNT[2],CNT[3],CNT[4],CNT[5],CNT[6],exp_mem[0],exp_mem[1],exp_mem[2],exp_mem[3],exp_mem[4],exp_mem[5]);
...
initial begin
  #(`MAX_CYCLE);
  $display("ERROR! Your system is Deadlock!");
  $finish;
end
//-------------------------------------------------------------------------------------------------------------------
